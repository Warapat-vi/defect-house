...
const LocationDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${config.apiUrl}/api/location/${id}`);
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`${config.apiUrl}/api/location/${id}`, { status, name });
  ...
  return (
    <Container>
      <Header as='h2'>Lcation Detail</Header>
      <Form>
        <Form.Field>
          <label>Name</label>
          <input 
